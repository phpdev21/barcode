<?php

use Illuminate\Http\Request;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

Route::middleware('auth:api')->get('/user', function (Request $request) {
    return $request->user();
});

Route::group(['prefix'=>'v1','namespace' => 'Api'], function(){
    Route::post('/sign-up','AuthenticateController@signUp');
    Route::get('/resend/verify/email','AuthenticateController@emailVerify');
    Route::get('/regenerate/token','AuthenticateController@regenerateToken');
    Route::post('/login','AuthenticateController@login');
    Route::post('/forget/password','AuthenticateController@forgetPassword');
    Route::get('/user/details','AuthenticateController@getDetails');
    Route::put('/user/update','AuthenticateController@updateUser');
    Route::put('/user/change/password','AuthenticateController@changePassword');
});
